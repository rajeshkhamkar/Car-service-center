...
<%
	...
	if((request.getParameter("act")).equals("Save"))
	{
		...
		if((request.getParameter("type_id")).equals(""))
		{
			...
			response.sendRedirect("../report-type.jsp?emp_id="+emp_id+"&msg= Category Saved Successfully");
		}
		else
		{
			results.put("type_id",request.getParameter("type_id"));
			out.println(typeObj.updateType(results));
			response.sendRedirect("../report-type.jsp?emp_id=0&msg=Category Updated Successfully");
		}
	}
%>
